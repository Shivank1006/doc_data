// --- Prompts for Gemini ---
export const JSON_EXTRACTION_PROMPT = `Analyze the provided page image and generate a structured JSON representation of its content.
...
7.  Numbered Images: Pay special attention to the numbered images indicated by red bounding boxes (e.g., #1, #2). There are {num_images} images found in total, indexed from 1 to {max_image_index}. When you encounter a numbered image in the document flow, insert an element object with:
    ...
    *   "image_id": N (where N is the integer number from the red box, e.g., 1, 2, 3)
...
export const TXT_EXTRACTION_PROMPT = `Analyze the provided page image and extract its text content.
...
export const HTML_EXTRACTION_PROMPT = `Analyze the provided page image and generate a semantic HTML5 representation of its content.
...
export const MARKDOWN_EXTRACTION_PROMPT = `Analyze the provided page image and generate a structured representation of its content using Markdown format.
