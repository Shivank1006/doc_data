...
// For development/testing, use dummy API keys if not provided
const API_KEY = VISION_PROVIDER === 'openai'
  ? (process.env.OPENAI_API_KEY || 'dummy-openai-key-for-testing')
  : (process.env.GEMINI_API_KEY || 'dummy-gemini-key-for-testing');

const LLM_MODEL_NAME = VISION_PROVIDER === 'openai'
  ? (process.env.OPENAI_MODEL_NAME || 'gpt-4-vision-preview')
  : (process.env.GEMINI_MODEL_NAME || 'gemini-pro-vision');
...
export const handler = async (event: any) => {
  try {
    // Each invocation processes a single page
    const { imageS3Uri, textS3Uri, pageNumber, runUuid, outputFormat, originalBaseFilename } = event;

    // Convert camelCase to snake_case for the handleRequest function
    const result = await handleRequest({
      s3_page_image_uri: imageS3Uri,
      s3_page_text_uri: textS3Uri,
    ...
    // Parse the response body to get the result URI
    if (result.statusCode === 200 && result.body) {
      try {
        ...
        return {
          ...
          pageNumber,
          resultS3Uri: responseBody.s3_result_uri,
          runUuid,
          outputFormat,
    ...
    console.error('Error in processor handler:', error);
    throw error;
  }
};
